...
std::shared_ptr<AstNode> Sema::SemaNumberExprNode(Token tok, int value, std::shared_ptr<CType> ty)
{
...
std::shared_ptr<AstNode> Sema::SemaStringExprNode(Token tok, std::shared_ptr<CType> ty)
{
    auto factor = std::make_shared<StringExpr>();
    factor->tok = tok;
    factor->ty = ty;
