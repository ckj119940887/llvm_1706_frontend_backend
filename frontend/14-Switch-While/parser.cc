...
bool Parser::ParseInitializer(std::vector<std::shared_ptr<VariableDecl::InitValue>> &arr, std::shared_ptr<CType> declType, std::vector<int> &offsetList, bool hasLBrace)
{
    ...
    if (tok.tokenType == TokenType::string && IsStringArrayTy(declType))
    {
    ...
    else
    {
        ...
        auto initValue = sema.SemaDeclInitValue(declType, node, offsetList, tok);
...
bool Parser::ParseStringInitializer(std::vector<std::shared_ptr<VariableDecl::InitValue>> &arr, std::shared_ptr<CType> declType, std::vector<int> &offsetList)
{
    ...
    int count = arrTy->GetElementCount();
    if (count < 0)
    {
        count = strTok.strValue.size();
        arrTy->SetElementCount(count);
    }

    int strCount = strTok.strValue.size();
    // 字符串越界
    if (count < strCount)
    {
        GetDiagEngine().Report(llvm::SMLoc::getFromPointer(tok.ptr), diag::err_large_len);
    }

    for (int i = 0; i < strCount; i++)
    {
        auto node = sema.SemaNumberExprNode(strTok, strTok.strValue[i], CType::IntType);
        offsetList.push_back(i);
        auto initValue = sema.SemaDeclInitValue(arrTy->GetElementType(), node, offsetList, strTok);
        offsetList.pop_back();
        arr.push_back(initValue);
    }

    for (int i = strCount; i < count; i++)
    {
        auto node = sema.SemaNumberExprNode(strTok, 0, CType::IntType);
        offsetList.push_back(i);
        auto initValue = sema.SemaDeclInitValue(arrTy->GetElementType(), node, offsetList, strTok);
        offsetList.pop_back();
        arr.push_back(initValue);
...
std::shared_ptr<CType> Parser::DirectDeclaratorFuncSuffix(Token iden, std::shared_ptr<CType> baseType, bool isGlobal)
{
    ...
    return std::make_shared<CFuncType>(baseType, params, llvm::StringRef(iden.ptr, iden.len), isVarArgs);
}
...
std::shared_ptr<AstNode> Parser::ParseDefaultStmt() {
    ...
    SwitchStmt *s = llvm::dyn_cast<SwitchStmt>(switchNodes.back().get());
    if(s->defaultStmt) {
...
std::shared_ptr<AstNode> Parser::ParsePrimary()
{
    ...
    {
        auto expr = sema.SemaStringExprNode(tok, tok.ty);
        Advance();
        return expr;
    }
    else
...
void Parser::ConsumeTypeQualifier()
{
    while (tok.tokenType == TokenType::kw_extern ||
