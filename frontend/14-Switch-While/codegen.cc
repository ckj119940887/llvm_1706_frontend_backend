...
llvm::Value *CodeGen::VisitIfStmt(IfStmt *p)
{
    ...
    irBuilder.CreateCondBr(condVal, thenBB, elseBB ? elseBB : lastBB);

    thenBB->insertInto(curFunc);
    irBuilder.SetInsertPoint(thenBB);
    p->thenNode->Accept(this);

    auto *tempBB = irBuilder.GetInsertBlock();
    if (tempBB->empty() || !tempBB->back().isTerminator())
    {
...
llvm::Value *CodeGen::VisitDoWhileStmt(DoWhileStmt *p) {
    ...
    condBB->insertInto(curFunc);
    irBuilder.SetInsertPoint(condBB);
    llvm::Value *val = p->cond->Accept(this);
...
llvm::Value *CodeGen::VisitCaseStmt(CaseStmt *p) {
    ...
    llvm::Value *caseValue = p->expr->Accept(this);

    CastValue(caseValue, switchInst->getCondition()->getType());

    auto *caseBB = llvm::BasicBlock::Create(context, "case.bb");
...
llvm::Value *CodeGen::VisitStringExpr(StringExpr *stringExpr)
{
    llvm::Constant *c = llvm::ConstantDataArray::getString(context, stringExpr->tok.strValue);
    llvm::GlobalVariable *g = new llvm::GlobalVariable(*module, c->getType(), true, llvm::GlobalValue::LinkageTypes::PrivateLinkage, c, stringExpr->tok.strValue);
    return g;
}
