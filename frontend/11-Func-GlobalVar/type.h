...
public:
    ...
    static llvm::StringRef GenAnnoyRecordName(TagKind tagKind);
};
...
public:
    ...
    std::shared_ptr<CType> GetBaseType() { return baseType; }
...
public:
    CArrayType(std::shared_ptr<CType> elementType, int elementCount) : CType(Kind::TY_Array, elementCount * elementType->GetSize(), elementType->GetAlign()), elementType(elementType), elementCount(elementCount) {}

    std::shared_ptr<CType> GetElementType() { return elementType; }
...
public:
    ...
    const std::vector<Member> GetMember()
    {
        return members;
    }
...
public:
    CFuncType(std::shared_ptr<CType> retType, std::vector<Param> &params, llvm::StringRef name);
