...
public:
    ...
    std::shared_ptr<AstNode> SemaNumberExprNode(Token tok, int value, std::shared_ptr<CType> ty);
    std::shared_ptr<AstNode> SemaStringExprNode(Token tok, std::shared_ptr<CType> ty);
    ...
    std::shared_ptr<CType> SemaAnnoyTagDecl(const std::vector<Member> &members, TagKind tagKind);

    std::shared_ptr<AstNode> SemaFuncDecl(Token tok, std::shared_ptr<CType> type, std::shared_ptr<AstNode> blockStmt);
    std::shared_ptr<AstNode> SemaFuncCall(std::shared_ptr<AstNode> left, std::vector<std::shared_ptr<AstNode>> &params);
